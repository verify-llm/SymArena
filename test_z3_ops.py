...
def test_z3_selfattn_tp():
    ...
    qkv_proj_shape_half = (3 * h * d // 2, E)
    qkv_bias_shape_half = (3 * h * d // 2,)
    out_proj_shape_half = (E, h * d // 2)
    query2 = create_tensor(query_shape, "query2", z3.Real)
    qkv_proj2a = create_tensor(qkv_proj_shape_half, "qkv_proj2a", z3.Real)
    qkv_proj2b = create_tensor(qkv_proj_shape_half, "qkv_proj2b", z3.Real)
    qkv_bias2a = create_tensor(qkv_bias_shape_half, "qkv_bias2a", z3.Real)
    qkv_bias2b = create_tensor(qkv_bias_shape_half, "qkv_bias2b", z3.Real)
    out_proj2a = create_tensor(out_proj_shape_half, "out_proj2a", z3.Real)
    out_proj2b = create_tensor(out_proj_shape_half, "out_proj2b", z3.Real)
    input_eq = (
        equalize_tensors([query1, query2])
        + equalize_tensors([qkv_proj1, np.concatenate([qkv_proj2a, qkv_proj2b], 0)])
        + equalize_tensors([qkv_bias1, np.concatenate([qkv_bias2a, qkv_bias2b], 0)])
        + equalize_tensors([out_proj1, np.concatenate([out_proj2a, out_proj2b], 1)])
    )
